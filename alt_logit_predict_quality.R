...
persons <- merge(subset(maintainers, Date == '8282010'),
                 persons,
                 by.x = 'Maintainer',
                 by.y = 'Person',
                 all.x = TRUE)
...
packages <- merge(packages,
                  ...
                  by = 'Package',
                  all.x = TRUE)
packages$PackagesMaintaining <- ifelse(is.na(packages$PackagesMaintaining), 0, packages$PackagesMaintaining)
...
design.matrix <- transform(design.matrix, LogPackagesMaintaining = log(1 + PackagesMaintaining))

#logit.fit <- glm(Installed ~ LogDependencyCount + LogSuggestionCount + LogImportCount + LogViewsIncluding + CorePackage + RecommendedPackage + PackagesMaintaining,
...
pdf('graphs/logit_maintaining.pdf')
ggplot(design.matrix, aes(x = log(1 + PackagesMaintaining), y = Installed)) +
  geom_jitter() +
  geom_smooth(method = 'lm') +
  ylim(c(0, 1))
dev.off()
